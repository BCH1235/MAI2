...
midi2audio_spec = importlib.util.find_spec("midi2audio")
if midi2audio_spec:
    midi2audio_module = importlib.import_module("midi2audio")
    FluidSynth = getattr(midi2audio_module, "FluidSynth", None)
else:
    FluidSynth = None
...
def process_score():
    ...
    if uploaded_file and uploaded_file.filename.endswith('.pdf'):
        ...
        bundled_audiveris_lib = os.path.join(backend_dir, 'app-5.7.1', 'lib')
        audiveris_candidates = [
            os.getenv('AUDIVERIS_JAR_DIR') or os.getenv('AUDIVERIS_LIB_DIR'),
            bundled_audiveris_lib,
            r"C:\Program Files\Audiveris\app",
        ]
        audiveris_jar_path = next(
            (path for path in audiveris_candidates if path and os.path.isdir(path)),
            None,
        )

        if not audiveris_jar_path:
            raise FileNotFoundError(
                "Audiveris JAR 디렉터리를 찾을 수 없습니다. "
                "AUDIVERIS_JAR_DIR (또는 AUDIVERIS_LIB_DIR)를 .env에 설정해 주세요."
            )

        java_home = os.getenv("AUDIVERIS_JAVA") or os.getenv("JAVA_PATH")
        ...
        if env_java_home:
            java_candidates.extend(
                [
                    os.path.join(env_java_home, "bin", "java.exe"),
                    os.path.join(env_java_home, "bin", "java"),
                ]
            )

        jar_parent = os.path.dirname(audiveris_jar_path)
        java_candidates.extend(
            [
                os.path.join(jar_parent, "runtime", "bin", "java.exe"),
                os.path.join(jar_parent, "runtime", "bin", "java"),
                r"C:\Program Files\Audiveris\runtime\bin\java.exe",
                r"C:\Program Files\Audiveris\runtime\bin\java",
                "java",
            ]
        )
        ...
        for candidate in java_candidates:
            ...
            if candidate_str.lower() == "java":
                which_java = shutil.which("java")
                if which_java:
                    java_executable = which_java
                    break
                continue
            if os.path.exists(candidate_str):
                java_executable = candidate_str
                break

        if not java_executable:
            raise FileNotFoundError(
                "Java 실행 파일을 찾을 수 없습니다. AUDIVERIS_JAVA 경로를 .env에 지정해 주세요."
            )
        ...
        try:
            ...
            print(f"사용할 Java 경로: {java_executable}")
            
            jar_files = [
                os.path.join(audiveris_jar_path, file_name)
                for file_name in os.listdir(audiveris_jar_path)
                if file_name.endswith('.jar')
            ]
            if not jar_files:
                raise FileNotFoundError(
                    f"'{audiveris_jar_path}' 안에서 사용할 Audiveris JAR 파일을 찾을 수 없습니다."
                )
            ...
            if output_format == 'midi':
                ...
                try:
                    ...
                    if FluidSynth is None:
                        raise RuntimeError("FluidSynth 가 설치되어 있지 않습니다. midi2audio 패키지를 확인하세요.")
